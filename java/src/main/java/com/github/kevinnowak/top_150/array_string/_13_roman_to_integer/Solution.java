package com.github.kevinnowak.top_150.array_string._13_roman_to_integer;
...
class Solution {
  ...
  private static boolean isSubstractionCase(char firstCharacter, char secondCharacter) {
    return isSubstractionPossible(firstCharacter)
        && checkSubstraction(firstCharacter, secondCharacter);
  }
   ...
   * @return The integer representation of the roman number. @Constraints: 1 <= s.length <= 15 | s
   *     contains only the characters ('I', 'V', 'X', 'L', 'C', 'D', 'M') | It is guaranteed that s
  ...
  public int romanToInt(String s) {
    ...
    for (int i = 0; i < s.length(); i++) {
      ...
      if (secondCharacter != '\u0000'
          && romanToIntMap.containsKey(firstCharacter + String.valueOf(secondCharacter))) {
  ...
  //            }
  //
  //            if (secondCharacter != '\u0000' && isSubstractionCase(firstCharacter,
  // secondCharacter)) {
  //                result += getIntForRomanCharacter(secondCharacter) -
  // getIntForRomanCharacter(firstCharacter);
  //                i++;
  //            } else {
